...
export default function Reviews({ connection }) {
    ...
    const formattedConnection = typeof connection === 'object' ? connection: JSON.parse(connection);
      const flooredRating = Math.round(formattedConnection.rating.date.total / formattedConnection.rating.date.count);
    return <div>
        <div className=" m-auto min-w-[57rem] h-[30rem] ml-40 overflow-y-auto mt-28  items-center bg-white px-4 pb-8 pt-14 shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8 rounded-xl w-3/4">
            <div className='w-100 mx-auto flex mb-10'>
                <div className='w-1/4'>
            <Doughnut data={chartConfig} />
                </div>
 <div className='block mx-auto '>
                <h1 className='text-[8rem] text-black text-center flex w-100 space-x-5'>{flooredRating}
            ...
            <div className='mt-5'>
                {
                    currentMongoConnection.date &&
                        currentMongoConnection.date.shown.bothShown &&
