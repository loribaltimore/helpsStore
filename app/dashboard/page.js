...
// console.log(metricsByAgeObject, 'IS WORKING');
    return { likedPercentage, rejectedByPercentage, looksRating, dateRating, looksMetrics};
};

export default async function Dashboard() {
    const session = await getServerSession(authOptions);
    const { likedPercentage, rejectedByPercentage, looksRating, dateRating, looksMetrics } = await getData(session.userId);
    ...
    return (
        <div class="grid grid-rows-2 grid-flow-col gap-2 p-5 h-[50rem]">
            ...
            <div className="bg-pink-400 rounded-xl text-center my-auto">
                <h1 className='text-[10rem]'>{looksRating+ '/10'}</h1>
                <p>Average Looks Rating</p>
            </div>
            <div className="bg-pink-400 rounded-xl text-center my-auto">
