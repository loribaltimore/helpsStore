...
module.exports.addToCart = async (currentUserId, cart, item) => {
    ...
    console.log('THIS IS CONFIG', img);
    if (!cart || !cart.items) {
        ...
        if (cartItems.indexOf(name) === -1) {
            newItem = new CartItem(name, price, config, img, code);
           ...
           typeof cart.total !== 'number' ? cart.total = price : cart.total += price;
...
module.exports.removeFromCart = async (currentUserId, cart, item) => {
    let cartItems = cart.items.map(x => x.name);
