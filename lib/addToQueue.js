...
let addToQueue = async (cart) => {
    ...
    for (let i = 0; i < toDonate.length; i++){
        await activeUser.addDonation(toDonate[i].qty, activeUser._id);
        newDonation.transaction.total += toDonate[i].qty;
        ...
        if (activeUser.charities.donatedTo.get(toDonate[i].name) === undefined) {
            ...
            console.log(activeUser.charities.donatedTo.get(toDonate[i].name));
            await officialQueue.addToHistory(toDonate[i].name, toDonate[i].qty);
            if (toDonate[i].name === 'helps Pool') {
            await officialQueue.addToPool(toDonate[i].qty,
                activeUser.bio.firstName + ' ' + activeUser.bio.lastName, activeUser._id);
            }
        };
    };
